...
response = session.post("http://127.0.0.1:8180/admin/realms/", data=realm)
...
response = session.post(
    "http://127.0.0.1:8180/admin/realms/mwdb-oidc-dev/users", data=json.dumps(user)
)
if response.status_code != 409:
    response.raise_for_status()

response = session.get(
    "http://127.0.0.1:8180/admin/realms/mwdb-oidc-dev/users?username=foo"
)
...
response = mwdb_session.post(
    ...
    json={
        ...
        "client_id": "mwdb",
        "client_secret": "",
        "authorization_endpoint": "http://127.0.0.1:8180/realms/mwdb-oidc-dev/protocol/openid-connect/auth",
        "userinfo_endpoint": "http://keycloak.:8180/realms/mwdb-oidc-dev/protocol/openid-connect/userinfo",
        "token_endpoint": "http://keycloak.:8180/realms/mwdb-oidc-dev/protocol/openid-connect/token",
        "jwks_endpoint": "http://keycloak.:8180/realms/mwdb-oidc-dev/protocol/openid-connect/certs",
        "logout_endpoint": "http://127.0.0.1:8180/realms/mwdb-oidc-dev/protocol/openid-connect/logout",
    },
)
response.raise_for_status()
print("[+] Done!")

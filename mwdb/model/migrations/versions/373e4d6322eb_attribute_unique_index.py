...
def upgrade():
    ...
    op.execute('CREATE UNIQUE INDEX ix_attribute_unique ON attribute (key, md5(value::text));')
    op.drop_index('ix_attribute_value', table_name='attribute')
    op.create_index('ix_attribute_value', 'attribute', ['value'], unique=False, postgresql_using='gin')


def downgrade():
    op.drop_index('ix_attribute_unique', table_name='attribute')
    op.create_unique_constraint('attribute_object_id_key_value_key', 'attribute', ['object_id', 'key', 'value'])
    op.drop_index('ix_attribute_value', table_name='attribute')

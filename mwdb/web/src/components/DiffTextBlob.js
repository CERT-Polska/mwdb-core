...
class DiffTextBlobContentPresenter extends Component {
    ...
    getDiff(current, previous) {
        function moveCursor(cursor, payload) {
            ...
            let newCursor = lines.length === 1 ? 
            [cursor[0] + lines[0].length, cursor[1]] : 
            [lines[lines.length - 1].length, cursor[1] + (lines.length - 1)]
            return newCursor;
        }
        ...
        for(let mark of diff) {
            ...
            {
                let start = cursors[1], end = newCursors[1] = moveCursor(cursors[1], text);
                markers[1].push({
                    startCol: start[0],
                    startRow: start[1],
                    endCol: end[0],
            ...
            {
                let start = cursors[0], end = newCursors[0] = moveCursor(cursors[0], text);
                markers[0].push({
                    startCol: start[0],
                    startRow: start[1],
                    endCol: end[0],
            ...
            {
                rowMappings[0][row] = cursors[1][1] + (cursors[1][1] === newCursors[1][1] ? 0 : row - cursors[0][1])
            }
    ...
    componentDidMount() {
        ...
        split.getEditor(0).getSelection().on('changeCursor', () => {
            let sel0 = split.getEditor(0).getSelection();
            let sel1 = split.getEditor(1).getSelection();
            if(this.rowMappings[0][sel0.lead.row] !== undefined && this.rowMappings[0][sel0.lead.row] !== sel1.lead.row)
                split.getEditor(1).gotoLine(this.rowMappings[0][sel0.lead.row] + 1)
        })
    }
...
class DiffView extends Component {
    ...
    async updateTextBlob() {
        try {
            let currentBlob = await api.getObject("blob", this.props.match.params.current);
            let previousBlob = await api.getObject("blob", this.props.match.params.previous);
            this.setState({
                                ...
                                <div className="col-md-12">
                                    <table className="table table-striped table-bordered table-hover"
                                        ...
                                        <tbody>
                                            <tr>
                                                <th style={{"width": "49%"}}>
                                                    <Link to={`/blob/${this.state.current.id}`}>
                                                        {this.state.current.id}
                                                    </Link>    
                                                </th>
                                                <td>
                                                    <Link to={`/diff/${this.state.previous.id}/${this.state.current.id}`}>
                                                        <button onClick={this.handleMalwareDownload} target="_self" className="btn btn-primary">
                                                            <FontAwesomeIcon icon="exchange-alt" pull="left" size="x"/>
                                                        </button>
                                                    </Link>
                                                </td>
                                                <td>
                                                    <Link to={`/blob/${this.state.previous.id}`}>

...
export default function RecentInnerRow(props) {
    const classNames = (
        props.narrowOnly ? ["d-lg-none", "d-flex"] : (
            props.wideOnly ? ["d-none", "d-lg-flex"] : ["d-flex"]
        )
    ).concat(
        props.copyable ? ["flickerable"] : []
    )
    return (
        <div className={classNames.join(' ')}>
            ...
            </div>
            {
                props.copyable ? ( 
                    <span className="ml-2">
                        <ActionCopyToClipboard text={props.value} tooltipMessage="Copy to clipboard"/>
                    </span>
                ) : []
            }
        </div>

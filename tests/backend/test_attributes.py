...
def test_metakey_add(admin_session):
    ...
    admin_session.add_attribute(sample_id, attr_name, "random_value")
    attrs = admin_session.get_attributes(sample_id)["metakeys"]
    assert len(attrs) == 1
    assert attrs[0]["key"] == attr_name
    assert attrs[0]["value"] == "random_value"


def test_metakey_permissions(admin_session, attr_session):
    ...
    attrs = admin_session.get_attributes(sample_id)["metakeys"]
    assert len(attrs) == 1
    attrs = attr_session.get_attributes(sample_id)["metakeys"]
    assert len(attrs) == 0
...
def test_metakey_public_perm(admin_session, attr_session):
    sample_id = attr_session.add_sample()["id"]
    attr_name = random_name().lower()
    admin_session.add_attribute_definition(attr_name, "")
    admin_session.add_attribute_permission(
...
def test_metakey_hidden(admin_session, attr_session):
    sample_id = attr_session.add_sample()["id"]
    attr_name = random_name().lower()
    ...
    attrs = admin_session.get_attributes(sample_id, show_hidden=True)["metakeys"]
    assert len(attrs) == 1
...
def test_metakey_hidden_search(admin_session, attr_session):
    sample_id = attr_session.add_sample()["id"]
    attr_name = random_name().lower()
    admin_session.add_attribute_definition(attr_name, "", hidden=True)
    admin_session.add_attribute_permission(
...
def test_metakey_definition_remove(admin_session, attr_session):
    sample_id = attr_session.add_sample()["id"]
    attr_name = random_name().lower()
